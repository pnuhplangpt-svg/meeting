import { createHash, createHmac, timingSafeEqual, randomUUID, scryptSync, randomBytes } from 'node:crypto';
...
const GET_ACTION_ALLOWLIST = new Set([
  ...
  'getHealth'  // P3-5: 헬스체크 (인증 불필요, DB ping 포함)
]);
...
const ADMIN_TOKEN_TTL_SEC = 4 * 60 * 60;
const AUTH_WINDOW_MS = 10 * 60 * 1000;
const AUTH_MAX_ATTEMPTS = 10;
const AUTH_LOCK_MS = 5 * 60 * 1000;

// P3-1: scrypt 파라미터 (N=2^14, 메모리 ~16 MB/해시, Vercel 512 MB 이내)
...
function validateEnv() {
  const required = ['SUPABASE_URL', 'SUPABASE_SERVICE_ROLE_KEY', 'PROXY_TOKEN_SECRET', 'PROXY_PASSWORD_PEPPER', 'PROXY_ADMIN_CODE'];
...
function getClientIp(req) {
  // x-real-ip is set by Vercel's edge network and is harder to spoof than x-forwarded-for
...
function pruneRateLimitStore() {
  ...
  for (const [key, item] of store) {
    if (now - item.startedAt >= RATE_LIMIT_WINDOW_MS) {
      store.delete(key);
    }
  }
}

function pruneAuthAttemptStore() {
  const store = getAuthAttemptStore();
  const now = Date.now();
  for (const [key, item] of store) {
    const windowExpired = !item.startedAt || now - item.startedAt >= AUTH_WINDOW_MS;
...
function shouldServeGetFromSupabase(action) {
  ...
  if (action === 'getHealth') return true;  // P3-5
  return false;
}
...
// P3-1: scrypt 기반 해시 생성 (신규 예약 / 검증 후 업그레이드에 사용)
// 저장 형식: 'scryptv1:<salt_hex>:<hash_hex>'
function hashPassword(password) {
  const pepper = String(process.env.PROXY_PASSWORD_PEPPER || '').trim();
  const salt = randomBytes(SCRYPT_SALT_LEN);
  const hash = scryptSync(String(password) + '|' + pepper, salt, SCRYPT_KEY_LEN, {
    N: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P
  });
  return 'scryptv1:' + salt.toString('hex') + ':' + hash.toString('hex');
}

// P3-1: scryptv1 / 레거시 SHA256 모두 검증
function verifyPasswordHash(password, storedHash) {
  const pepper = String(process.env.PROXY_PASSWORD_PEPPER || '').trim();
  const stored = String(storedHash || '');

  if (stored.startsWith('scryptv1:')) {
    const parts = stored.split(':');
    if (parts.length !== 3) return false;
    try {
      const salt = Buffer.from(parts[1], 'hex');
      const expectedHash = Buffer.from(parts[2], 'hex');
      if (expectedHash.length !== SCRYPT_KEY_LEN) return false;
      const inputHash = scryptSync(String(password) + '|' + pepper, salt, SCRYPT_KEY_LEN, {
        N: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P
      });
  ...
  if (/^[0-9a-f]{64}$/.test(stored)) {
    const legacyHash = createHash('sha256').update(String(password) + '|' + pepper, 'utf8').digest('hex');
    try {
      return timingSafeEqual(Buffer.from(stored, 'hex'), Buffer.from(legacyHash, 'hex'));
    } catch (e) {
      return false;
    }
  }
...
// P3-1: SHA256 레거시 해시 여부 판별 (업그레이드 대상)
...
function listRecentDateKeys(days) {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    keys.push(d.toLocaleString('en-CA', { timeZone: 'Asia/Seoul' }).slice(0, 10));
  }
  return keys;
}
...
async function buildOperationalMetricsFromSupabase(config, windowDays) {
  ...
  const nowDay = new Date().toLocaleString('en-CA', { timeZone: 'Asia/Seoul' }).slice(0, 10);
...
async function handleSupabaseGetAction(action, query, context) {
  ...
  // P3-5: 헬스체크 — DB ping + 환경변수 상태 (인증 불필요)
  if (action === 'getHealth') {
    let dbOk = false;
    try {
      await supabaseSelect(config, 'rooms', { select: 'id', limit: '1' });
      dbOk = true;
    } catch (e) {
      // DB ping 실패
    }
    ...
    return {
      ...
      body: {
        success: true,
        data: {
          status: dbOk && missingEnv.length === 0 ? 'ok' : 'degraded',
          db: dbOk,
...
async function handleSupabasePostAction(action, body, context) {
  ...
  if (action === 'createReservation') {
    ...
    // P1-7: KST 기준 과거 날짜 차단
    const todayKst = new Date().toLocaleString('en-CA', { timeZone: 'Asia/Seoul' }).slice(0, 10);
    if (safeDate < todayKst) {
      return { handled: true, status: 200, body: { success: false, error: '과거 날짜에는 예약할 수 없습니다.' } };
    }
  ...
  if (action === 'verifyPassword') {
    ...
    if (!isPlaceholderHash(row.password_hash)) {
      // P3-1: scryptv1 / 레거시 SHA256 모두 검증
      matched = verifyPasswordHash(password, row.password_hash);
    ...
    // P3-1: 검증 성공 시 레거시 SHA256 해시 → scrypt 자동 업그레이드 (best-effort)
    if (isSha256Hash(row.password_hash)) {
      try {
        await supabaseRequest(config, 'PATCH', 'reservations', { id: 'eq.' + id }, {
          password_hash: hashPassword(password)
        });
        await appendAuditLogSafe(config, 'verifyPassword', 'info', 'system', id, 'password hash upgraded to scryptv1');
      } catch (e) {
        // 업그레이드 실패는 비치명적: 다음 검증 시 SHA256으로 재시도 가능
      }
    }
...
export default async function handler(req, res) {
  ...
  if (missingEnv.length > 0) {
    return res.status(500).json({ success: false, error: '서버 환경변수가 올바르게 설정되지 않았습니다: ' + missingEnv.join(', ') });
  }
