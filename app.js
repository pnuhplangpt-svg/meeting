...
function bindUiActions() {
  ...
  // P1-1: 모달 확인 버튼 단일 리스너 (재등록 방지)
  var modalPasswordConfirm = byId('modalPasswordConfirm');
  if (modalPasswordConfirm) modalPasswordConfirm.addEventListener('click', confirmPasswordModal);

  var modalDeleteConfirm = byId('modalDeleteConfirm');
  if (modalDeleteConfirm) modalDeleteConfirm.addEventListener('click', confirmDeleteModal);

  var adminDeleteConfirmBtn = byId('adminDeleteConfirmBtn');
  if (adminDeleteConfirmBtn) adminDeleteConfirmBtn.addEventListener('click', handleAdminDeleteConfirm);
