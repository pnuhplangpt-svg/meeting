...
- [x] 월간 지표 API/UI 반영: 최근 30일 예약/인증/회의실 변경 지표를 관리자 모달에서 조회 가능하도록 구현
- [x] 월간 지표 알림/리포트 자동화 반영: Script Property 기반 수신자/임계치 설정, 관리자 수동 발송, 트리거용 자동 발송 함수 추가
- [x] 월간 지표 주간 추이 대시보드 반영: 최근 30일 일자별 인증실패/예약생성 추이와 7일 이동평균을 관리자 모달에서 확인 가능하도록 구현
- [x] 월간 지표 이상치 탐지 규칙 반영: 인증실패 연속 증가(3일+) 및 급증(최근 중앙값 대비 2배↑, +3건↑) 탐지/표시/리포트 반영

> 다음 구현 순서: 운영 체크리스트(실배포) 반복 적용
...
- [x] Epic C 완료
- [x] Epic D 완료
- [ ] 통합 회귀 1회 통과 *(mock E2E 스크립트는 준비됨. 현재 환경에서는 Playwright 패키지 설치가 프록시 제한으로 실패하여 실행 검증 보류)*
- [x] 운영/보안 고도화 항목 후속 반영 계획 확정 *("다음 구현 순서"에 운영 체크리스트 반복 적용/이상치 탐지 규칙 명시)*
